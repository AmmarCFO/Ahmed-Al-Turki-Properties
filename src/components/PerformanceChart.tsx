          ...
          <ResponsiveContainer width="100%" height="100%">
            {chartType === 'financials' ? (
              <ComposedChart data={chartData} margin={{ top: 10, right: 10, left: 10, bottom: 20 }}>
                ...
                <Line type="monotone" dataKey="operatorFee" name={isAr ? 'حصة المشغل (15%)' : 'Operator Fee'} stroke="#725937" strokeWidth={2.5} dot={{ r: 4 }} />
              </ComposedChart>
            ) : (
              <ComposedChart data={chartData} margin={{ top: 10, right: 10, left: 10, bottom: 20 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#EDE5DC" vertical={false} />
                <XAxis dataKey="name" stroke="#8B6F47" fontSize={12} tickLine={false} />
                <YAxis domain={[80, 100]} stroke="#8B6F47" fontSize={11} tickLine={false} axisLine={false} tickFormatter={(val) => `${val}%`} />
                <Tooltip
                  contentStyle={{ backgroundColor: '#1d1d1f', borderRadius: '16px', color: '#fff', border: '1px solid #B8865F' }}
